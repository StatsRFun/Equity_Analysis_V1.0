...
if st.button("Run Analysis", type="primary"):
  with st.spinner(f"Fetching 60 months of data for {stock_ticker}..."):
    try:
      ...
      st.markdown(f"### 📰 Financial Context & News for {stock_ticker}...:  ** ${current_price:.2f}  ({pct_ch:+.2f}%) **     ")
      ...
      with tab3:
        ...
        ax.plot(dates, Y, label=f"{stock_ticker} Returns", color="darkorange", linewidth=1.5)
